...
def main():
    ...
    # pe-bands update <ticker> <min> <max>
    update_parser = pe_subparsers.add_parser("update", help="Update or create a PE band")
    update_parser.add_argument("ticker", help="Asset ticker (e.g. AAPL)")
    update_parser.add_argument("min", type=float, help="Minimum PE value")
    update_parser.add_argument("max", type=float, help="Maximum PE value")
    ...
    if args.command == "pe-bands":
        if args.subcommand == "ls":
            ...
            print("Usage: irm pe-bands {ls,update}")
    else:
        parser.print_help()
