...
*   **Gateway (`pi-mono-stack-gateway`)**
    ...
    *   **概要设计**: 参考 [Gateway 概要设计](gateway/DESIGN.md)。
*   **Redis**
    *   **职责**：消息总线与状态存储。
    *   **作用**：作为 Gateway、Agent、Dkron 之间的异步消息流（Stream）、发布订阅（Pub/Sub）通道的核心组件，同时也是 Dkron 任务调度器的数据后端。
    *   **数据协议**: 参考 [Gateway 协议规范](gateway/DESIGN.md#gateway-模块数据协议设计-data-protocol-design)。
*   **LiteLLM (`litellm`)**
...
*   **Dkron (`dkron`)**
    ...
    *   **作用**：负责全栈定时任务与自动化剧本的调度。它通过自定义 Executor（Background/Reminder）将执行结果实时映射到 Redis 消息中心。具备**系统级自治倾向**：其容器入口脚本会自动扫描并注册自维护任务（如监控 Shell 失败、清理过期 Job 记录），确保调度环境的长期健康。同时作为系统启动链的“健康网关”，确保其 API 就绪后下游依赖工具才开始启动。
    *   **概要设计**: 参考 [Dkron 概要设计](dkron/DESIGN.md)。
*   **Llama Server (`llama-server` & `llama-embedding`)**
    *   **职责**：边缘 AI 推理与文本向量化引擎。
    *   **作用**：基于 `llama.cpp` 的高性能 CUDA 推理服务。
        *   `llama-server`: 提供符合 OpenAI 标准的本地聊天/内容生成 API，用于网关任务摘要及 Agent 本地推理。
        *   `llama-embedding`: 提供本地文本向量化服务，支持高效的语义检索。
        

### 2. 长程记忆与检索服务
*   **Memory Service (`memory`)**
    *   **职责**：对话级原子记忆引擎 (Conversational Memory Agent)。
    *   **作用**：基于 **Mem0 SDK** 自动从 `agent_in/out` 消息流中提取、去重并沉淀用户事实。
        *   **观察者模式**：异步监听对话，利用本地 `llama-server` 提取原子事实。
        *   **存储**：使用 ChromaDB 维护向量索引（512 维），由本地 `llama-embedding` 提供支持。
        *   **接入**：为 Agent 提供 `memory` CLI 工具和 HTTP 检索接口。
    *   **概要设计**: 参考 [Memory 概要设计](memory/DESIGN.md)。
...
*   **swe**：软件工程工具，允许 Agent 操作 GitHub（抓取 Issue、提交 PR 等）、Git 和 Docker。
*   **twitter**：Twitter API 交互模块，支持 Agent 发布推文与进行社交监听。
*   **wikipedia**：对 Wiki 词条进行查阅和信息获取的外部技能。

### 4. 可观测性与遥测服务 (Observability & Telemetry)
*   **OpenObserve (`openobserve`)**
    *   **职责**：轻量级、高性能的统一可观测性平台（Datadog 替代方案）。
    *   **作用**：基于 Rust 编写的高效日志、指标和分布式追踪 (Traces) 存储引擎及可视化数据大盘。充当整个系统的“监控大脑”。
*   **OTEL Collector (`otel-collector`)**
    *   **职责**：OpenTelemetry 遥测数据总线。
    *   **作用**：解耦数据源和后端。负责接收 LiteLLM 的追踪数据、主动抓取本地 `llama-server` 的 Prometheus 性能指标、并接收各应用的服务日志，统一加工处理后分发至 OpenObserve（同时亦可双写追踪数据至 Langfuse）。
...
1.  **`docker-compose.yml`**
    *   **位置**：根目录。
    *   **作用**：定义了所有容器服务的网络（以 `host` 模式为主提升内网通讯效率及穿透需求）、环境变量映射、持久化卷映射（主要是 `.pi` 目录与 `docker.sock`），确立了组件的物理拓扑。同时也支持通过环境变量（如 `OPENOBSERVE_PORT`, `OTEL_COLLECTOR_HTTP_PORT` 等）灵活调整服务端口。
2.  **`stack-ctl.sh`**
    *   **位置**：根目录。
    *   **作用**：系统的控制脚本。负责在启动整个栈时（`./stack-ctl.sh up`）从 macOS/Linux 的 `pass` 工具中安全提取各种 API Key（如 `GEMINI_API_KEY`, `TELEGRAM_TOKEN`），并将其作为环境变量注入到相应的容器中去，避免密钥在代码或明文配置中泄露。同时也提供 `build`, `down`, `logs`, `backup` 等操作。在进行 `backup` 时，会自动排除 `*.gguf` 等体积巨大的模型文件以节省空间。
...
4.  **.pi/agent/otel-collector-config.yaml**
    *   **位置**：用户挂载的 `.pi/agent` 配置目录。
    *   **作用**：OpenTelemetry Collector 配置文件。
    *   **详情**：定义了遥测数据的接收器（Receivers）、处理器（Processors）及导出器（Exporters）。它负责将各种组件（如 LiteLLM, llama-server）的 Trace、Metrics 和 Logs 统一汇聚并推送到 OpenObserve 后端。同样支持通过环境变量进行动态端口配置。
...
2.  它执行内置任务或特定脚本。特别是 `monitor-failed-shell-jobs` 任务，每 15 分钟会主动检查所有 Shell 任务的健康状态。
3.  **Dkron** 的运行情况和输出结果经专门的 Redis 信道写入 `background_out` (作业日志) 或 `reminder_out` (失败预警)。
4.  **Gateway** 捕获到这些外部调度事件之后，自动将作业报告或失败告警向系统管理员推送。
5.  当有更复杂的上下文查询诉求时，Agent 后台通过 **Memory** 的 SDK 查询过去的文档快照或记忆摘要片段，并将新思考后的见解通过 Voyage Embedding 并插入回 Milvus 库中形成循环。