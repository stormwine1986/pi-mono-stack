...
### 2. 自动化初始化 (Entrypoint & Gateway Setup)
容器启动与系统初始化：
1.  Dkron 容器通过 `entrypoint.sh` 启动 agent。
2.  `gateway` 启动时，通过 `setup.ts` 自动向 Dkron 注册/同步核心管理任务。
...
### 4. 任务治理 (Cleanup)
为了防止一次性任务（非周期任务）堆积，系统内建了清理机制：
*   **清理脚本**: `cleanup_once_jobs.sh`。
*   **清理逻辑**: 识别 `next` 调度为空且距离最后一次执行已超过 1 小时的任务。
*   **调度**: 由 Gateway 注册，每小时执行一次，自动调用 API 删除过期的已完成任务。
