...
*   **Dkron (`dkron`)**
    ...
    *   **作用**：负责全栈定时任务与自动化剧本的调度。它通过自定义 Executor（Background/Reminder）将执行结果实时映射到 Redis 消息中心，并内置了对 Shell 周期任务的自动扫描与失败预警机制。同时作为系统启动链的“健康网关”，确保其 API 就绪后下游依赖工具（如 `browser-use`, `swe` 等）才开始启动。
